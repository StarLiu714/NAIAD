...
def main():
    ...
    if dataset_root:
        ...
        def _resolve_ppm_paths(df):
            if "ppm_paths" not in df.columns:
                return df
            df = df.copy()
            df["dataset_root"] = dataset_root

            def resolve_ppm_paths_value(ppm_paths_value):
                if pd.isna(ppm_paths_value):
                    return ppm_paths_value

                ppm_paths = ast.literal_eval(str(ppm_paths_value))
                resolved_ppm_paths = []
                for ppm_paths_sublist in ppm_paths:
                    resolved_ppm_paths.append([
                        (
                            ppm_path if os.path.isabs(str(ppm_path))
                            else os.path.normpath(os.path.join(dataset_root, str(ppm_path)))
                        )
                        for ppm_path in ppm_paths_sublist
                    ])
                return repr(resolved_ppm_paths)

            mask = df["ppm_paths"].notna()
            df.loc[mask, "ppm_paths"] = df.loc[mask, "ppm_paths"].map(
                resolve_ppm_paths_value
            )
            return df

        df_valid = _resolve_structure_paths(df_valid)
        df_train = _resolve_structure_paths(df_train)
        df_valid = _resolve_ppm_paths(df_valid)
        df_train = _resolve_ppm_paths(df_train)
